...
from model import GIN, SimpleNNBaseline, GIN_MPML, GCN_MPML, GCN, GAT_MPML, GAT, GIN_MPML_Learned, GAT_MPML_Learned, GCN_MPML_Learned
...
def model_factory(model_cfg: dict, dataset):
    str_2_type = {
        'GCN_MPML': GCN_MPML_Learned,
        'GAT_MPML': GAT_MPML_Learned,
...
def experiment(exp_cfg, device, output_dir=None, verbose=True, output_cache=None):
    ...
    if exp_cfg['dataset_name'] == 'ogbg-molhiv':
        split_ds = []
        split_idx = dataset.get_idx_split()
        split_ds.append((dataset[split_idx["train"]], dataset[split_idx["test"]], dataset[split_idx["valid"]]))
        split_i = 0
    else:
    ...
    # np.random.seed(42)
    # sample_pts = np.random.randint(0, 100, size=(16, 2))
    # sample_pts_t = torch.tensor(sample_pts, dtype=torch.float, device=device, requires_grad=True)
...
def restart_experiment(exp_cfg, device, output_dir=None, verbose=True, output_cache=None):
    ...
    for fold_i, (train_split, test_split, validation_split) in enumerate(split_ds):
        ...
        print(f"Fold {fold_i} took {end-start} seconds")
        # break #todo remove!!!
        if not os.path.exists(output_dir):
            os.makedirs(output_dir, exist_ok=True)
...
if __name__ == '__main__':
    ...
    __model_cfg_meta = {
        ...
        'use_node_degree': False, # True only for IMDB-Binary else False
        'set_node_degree_uninformative': False,
        'pooling_strategy': 'sum',
        'use_node_label': False,
        'input_features_dim': 82,
        'use_node_features': True, # False for IMDB-Binary else True
        'gin_number': 1,
        'gin_dimension': 64,
