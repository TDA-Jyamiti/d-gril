...
class GAT(nn.Module):
    def __init__(self, 
        ...
        **kwargs,  
        ...
        cls_in_dim = sum([1 * dims[0] + num_head * dims[i] for i in range(1, len(dims))])
        self.concat_fp = concat_fp
        if self.concat_fp == 'maccs':
            cls_in_dim += 167
        elif self.concat_fp in ['ecfp','morgan2', 'morgan3']:
...
class GIN_MPML_Learned(nn.Module):
    ...
    def forward(self, batch):
        ...
        lmbda_0, lmbda_1 = self.mpml_0(f, edge_index)
...
class GIN_GRILHead(nn.Module):
    def __init__(self, 
        ...
        super().__init__()
        self.gin = GIN(dataset, **model_cfg)
        self.gin.load_state_dict(torch.load(model_pth))
        self.gin.use_as_feature_extractor = True
        self.mpml_0 = MultiPersDiff(res=0.01, hom_rank=0, step=10, l=2)
        self.num_centre_pts = int(1 / (10 * 0.01))
